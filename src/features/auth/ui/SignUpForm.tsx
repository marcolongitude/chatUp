...
import type { RegisterData } from "../model/auth";
import {
  ButtonContainer,
  ErrorText,
  FooterContainer,
...
export function SignUpForm({ onSubmit, onSignIn, isLoading = false, error }: SignUpFormProps) {
  ...
  return (
    <FormContainer behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : 20}>
      <ScrollContent contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        <Form>
          <LogoContainer>
            <LogoImage source={require("~/assets/logo-chatup.png")} contentFit="contain" cachePolicy="memory-disk" transition={200} />
          </LogoContainer>
          ...
          <ButtonContainer>
            <Button title={t("auth.signUp")} onPress={handleSubmit(handleFormSubmit)} variant="primary" loading={isLoading} />
          </ButtonContainer>
